...
void initLidar(int fileDesc)
{
	...
	RequestPacket packet=RequestPacket(rp_values::GET_HEALTH);
	uint8_t size=packet.get_packet(data);
	printf("packetsize %d", size);
	write(fileDesc, data, size);
	ssize_t nread=read(fileDesc, data, sizeof(uint8_t)*10);
	printf("read %d\n", (int)nread);
	packet=RequestPacket(rp_values::SET_PWM);
	packet.add_payload(0x94);
	packet.add_payload(0x02);
	size=packet.get_packet(data);
	printf("packetsize %d", size);
	write(fileDesc, data, size);
